...
of lexicon data originally available in a text-based format. The system consists of several components:
...
making it hard for new team members to actively take part
in further development.

\av{
Moreover, we have seen a sharp rise in the number of internal links between different lexical units within Vallex 
when phenomena such as lexical-semantic alternations and light verb constructions became the central topics of our research.
In addition to the old needs, it has now become necessary to watch out for changes in the data and make sure that the internal compatibility of the data does not become broken.
The old pipeline contained no mechanisms that could be efficiently used for this purpose.
This issue has become particularly pressing since Vallex now has three additional sister projects,
all linking back to its data:
NomVallex, a lexicon of deverbal nouns;
RU-Vallex, a lexicon of Russian verbs;
and PL-Vallex, a lexicon of Polish verbs.
}
...
\av{Note that besides the internal structure of attributes output by the parser (Section \ref{parser}),
...
\end{verbatim}
\av{would match all lexical units that failed the test shown in Figure \ref{fig:data-test-code}.
}
...
\subsection{User Features \av{\textbf{CELE NOVE}}}
...
as well as computed properties not present in the source files (\ref{sec:computed})
...
A simple interface for constructing histograms is also available (ibid.).
...
the client reads data from the working copy of the Subversion repository.
When the editor view is opened, 
the system stores information about the current state of the the entry for the given lexical unit in the in-memory representation (\emph{SQLite} database)
when the editor view is closed,
the system first checks that the state of the entry in the database has not changed since opening the editor,
then parses the new textual form of the lexical unit and replaces the whole unit in the in-memory representation.%
\footnote{Actually, the \emph{SQLite} database is also regularly saved to the disc in order to avoid loss of data in case of unexpected program failures.}
In case of an attempt to save edits on a unit that has changed in the database since the editing window was opened,
the user is warned and no changes are stored into the database.
Upon request, the content of database is output in the textual form back to the same place (making sure that any files changed in the mean-time are backed up;
see Section \ref{sec:output});
it is the users responsibility to submit the changes to Subversion,
...
\section{Future work}
\avout[Some features of our old pipeline,
e.g.\ xml and pdf output,
have yet to be implemented in PyVallex.
]{Currently PyVallex is missing some features of the previous system, which we plan to
incorporate: xml and pdf output, ...}
It is also only available in source code form\footnote{\url{https://gitlab.com/Verner/pyvallex}} and we plan to publish it as pre-built
package in the PyPi package repository. The client-side \av{\emph{Qt}-based} GUI is now distributed as a stand-alone
executable file. Although this has some advantages (simplicity, easy updates), it has a significant
performance cost---the single file is basically a self-extracting archive which must unpack itself
every time the program is run.

While the client-side \av{\emph{Qt}-based} GUI allows for basic editing, it would be nice to incorporate this into
the web UI. This will require some form of user authentication/authorization to be implemented.
Eventually, the editing capabilities could be extended to allow for more comprehensive
edits (including creating new lexemes/lexical units). This would allow \av{some} annotators \avout{who are not familiar
with the plain-text format} to use the PyVallex system as the main or only tool to create or edit 
lexicons\av{, and external users to efficiently submit feedback on individual lexical units as well as suggest extensions of the lexicon}.

\av{Finally, the modular architecture of PyVallex with database-based internal representation
makes it an ideal tool for script-based examination of multiple revisions of the same data.
